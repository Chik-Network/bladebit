...
ProofFetchResult PlotReader::FetchQualityXsForP7Entry( 
    ...
    const byte    challenge[BB_CHIK_CHALLENGE_SIZE], 
...
ProofFetchResult PlotReader::FetchQualityForP7Entry( 
    const uint64 t6Index, 
    const byte challenge [BB_CHIK_CHALLENGE_SIZE], 
          byte outQuality[BB_CHIK_QUALITY_SIZE] )
{
    uint64 x1, x2;
    ...
    const size_t HASH_SIZE_MAX = BB_CHIK_QUALITY_SIZE + CDiv( 2*50, 8 );
    byte hashInput[HASH_SIZE_MAX] = {};

    const uint32 k        = _plot.K();
    const size_t hashSize = BB_CHIK_QUALITY_SIZE + CDiv( 2*k, 8 );

    memcpy( hashInput, challenge, BB_CHIK_QUALITY_SIZE );

    Bits<HASH_SIZE_MAX-BB_CHIK_QUALITY_SIZE> hashBits;
    hashBits.Write( x2, k );
    hashBits.Write( x1, k );

    hashBits.ToBytes( hashInput + BB_CHIK_QUALITY_SIZE );
